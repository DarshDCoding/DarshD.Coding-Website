...
export default function FrontendCard({card}) {
    return (
        <Card
            key={card.title}
            className="relative w-fit p-0 border border-gray-650 rounded-t-xl"
        >
            ...
            <CardContent className="flex flex-col w-full gap-2">
                <CardTitle className="text-[25px] font-semibold font-primary">
                    {card.title}
                </CardTitle>

                <div id="tech-stack" className="w-full flex gap-2">
                    {card.techStack.map((techStack) => (
                        <span key={techStack} className="text-[15px] font-normal">{techStack}</span>
                    ))}
                </div>

                <CardDescription className="text-[12px] h-16">
                    {card.description}
                </CardDescription>
            </CardContent>
